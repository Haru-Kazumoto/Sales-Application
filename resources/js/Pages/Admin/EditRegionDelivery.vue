    ...
    <div class="d-flex flex-column gap-4">
        ...
        <div class="card shadow-sm border-0">
            <div class="card-body">
                ...
                <div class="d-flex">
                    <n-button class="ms-auto" type="primary" size="large" @click="handleSubmit">UPDATE WILAYAH</n-button>
                </div>
            </div>
        </div>
    </div>
...
export default defineComponent({
    setup () {
        const page = usePage();
        const region_delivery: any = page.props.regionDelivery;

        const form = useForm({
            region_name: region_delivery.region_name,
            region_code: region_delivery.region_code,
            region_price: region_delivery.region_price
        });

        function handleSubmit() {
            form.put(route('admin.region-delivery.update', region_delivery.id), {
                onSuccess: (page) => {
