<template>
    <div class="d-flex flex-column gap-4">
        <TitlePage title="Create new role" />
                ...
                <div class="card-body">
                    <form @submit.prevent="submit" class="d-flex flex-column gap-3">
                        <div>
                            ...
                            <n-input id="Nama Role" placeholder="" v-model:value="model.role_name" />
                            <ErrorInput :error="$page.props.errors['rol_name']" />
                        </div>
                        <n-button type="primary" attr-type="submit" class="w-25 my-3">Buat role baru</n-button>
                    </form>
                </div>
            </div>
        </div>
...
import { RoleCreateDTO, } from '../../../../types/dto';
...
export default defineComponent({
    setup() {
        ...
        const form = useForm<RoleCreateDTO>({
            role_name: ''
        });

        function handleSubmit() {
            form.post(route('admin.create-role.post'), {
                onSuccess: () => {
