    ...
    <div class="d-flex flex-column gap-4">
        ...
        <div class="row g-3">
            <div class="col-12 col-lg-6 d-flex align-items-lg-center gap-3">
                <label>Total Jumlah Gudang</label>
                <span class="border px-4 py-1 bg-white" style="border-radius: 3px;">
                    {{ ($page.props.storehouses as any).total }}
                </span>
            </div>
        ...
        <div class="card shadow-sm border-0 mb-5">
            <div class="card-body">
                <n-data-table :columns="columns" :bordered="false" :data="($page.props.storehouses as any).data"
                ...
                <div class="d-flex mt-3">
                    <n-pagination class="ms-auto" v-model:page="pagination.current_page"
                        ...
                        @update:page-count="pagination.last_page = ($page.props.storehouses as any).last_page" />
                </div>
            </div>
        </div>
    </div>
...
export default defineComponent({
    setup() {
                        ...
                        return h('div', { class: 'd-flex gap-2' }, [
                            h(
                                NButton,
                                {
                                    ...
                                    size: 'small',
                                    onClick() {
                                        router.visit(route('admin.storehouse.edit', row.id));
                                    }
                                },
                                { default: () => "EDIT" }
                            ),
                            h(
                                NButton,
                                {
                                    ...
                                    onClick() {
                                        Swal.fire({
                                            title: `Hapus Gudang ${row.name}?`,
                                            text: "Data akan dihapus secara permanen",
                                            icon: 'warning',
                                        }).then((result) => {
                                            if (result.isConfirmed) {
                                                router.delete(route('admin.storehouse.delete', row.id), {
                                                    onSuccess: () => {
                                                        notification.success({
                                                            title: "Berhasil menghapus data gudang",
                                                            duration: 1500,
                                                            closable: false,
                                                        });
                                                    },
        ...
        const handleSearch = () => {
            router.get(route('admin.create-storehouse'), {
        ...
        function handleSubmitUnit() {
            form.post(route('admin.storehouse.post'), {
                onError: () => {
        ...
        const pagination = reactive({
            current_page: (page.props.storehouses as any).current_page,
            per_page: (page.props.storehouses as any).per_page,
            total: (page.props.storehouses as any).total,
            last_page: (page.props.storehouses as any).last_page,
        });
